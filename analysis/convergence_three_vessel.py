...
def main() -> None:
    ...
    parser = set_arg_parser(
        description="Three-vessel convergence",
    ...
    model = hd.load_from_config("analysis/configs/three_vessel.json")
    model.set_inlet(
        vessel_id="parent",
        kind="flow_rate",
    ...
    if args.outlet_model == "capillary":
        for vessel_id in ("daughter1", "daughter2"):
            model.set_windkessel_outlet(
                vessel_id=vessel_id,
                bed_id=f"{vessel_id}_bed",
    ...
    output_dir = args.output_dir or Path(f"analysis/outputs/convergence_three_vessel/{name}")
