...
def test_two_vessel_dg_junction_rest_state_stays_rest() -> None:
    physics = make_physics(
        length=1.0,
        area0=0.126,
        beta=0.060606e7,
    ...
    upstream = make_dg_vessel(vessel_id="upstream", physics=physics)
    downstream = make_dg_vessel(vessel_id="downstream", physics=physics)
    ...
    network = VascularNetwork(
        ...
        external_boundaries={
            ...
            NetworkEndpoint("downstream", EndpointSide.RIGHT): CopyBoundaryCondition(),
        },
    )
    ...
    for vessel in (upstream, downstream):
...
def test_single_vessel_dg_network_with_small_fixed_dt_and_copy_boundaries_runs() -> None:
    ...
    vessel.state_n.A[:, :] = physics.params.area0 * (1.0 + 0.001 * np.sin(2.0 * np.pi * x))
